...
export default function PrintHotelList() {
  ...
  const thStyle: React.CSSProperties = { background: "#16a34a", color: "#fff", padding: "2.5mm 2.5mm", textAlign: "left", fontSize: "7pt", textTransform: "uppercase", letterSpacing: "0.5px", border: "1px solid #16a34a" };
          ...
          {(group.hotels?.makkah?.name || group.hotels?.madinah?.name) && (
            <div style={{ display: "flex", gap: "6mm", justifyContent: "flex-end", fontSize: "8pt", color: "#333", lineHeight: 1.7, marginTop: "-3mm", marginBottom: "4mm" }}>
              {group.hotels?.makkah?.name && (
                <div>Makkah 2: <b style={{ color: "#16a34a" }}>{group.hotels.makkah.name}</b>
                  {group.hotels.makkah.checkIn && <span style={{ color: "#888" }}> (Check-in: {group.hotels.makkah.checkIn})</span>}
                </div>
              )}
              {group.hotels?.madinah?.name && (
                <div>Madinah: <b style={{ color: "#16a34a" }}>{group.hotels.madinah.name}</b>
            ...
            <tbody>
              {pilgrims.map((p, i) => (
                <tr key={p.id} style={{ background: i % 2 === 0 ? "#fff" : "#f0fdf4" }}>
                  ...
                  <td style={{ ...tdStyle, fontWeight: 700, textAlign: "center", color: "#16a34a", fontSize: "10pt" }}>{p.roomNumber || "—"}</td>
