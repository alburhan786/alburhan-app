...
export default function PrintHajiList() {
  ...
  return (
    <>
      ...
      <div className="no-print" style={{ padding: "16px", background: "#f0fdf4", borderBottom: "2px solid #0d5040", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", flexWrap: "wrap" }}>
        ...
        <button onClick={handleDownload} disabled={pdfLoading} style={{ padding: "10px 24px", background: "#0d5040", color: "#fff", border: "none", borderRadius: "8px", fontWeight: 600, cursor: "pointer", opacity: pdfLoading ? 0.6 : 1 }}>
      ...
      <div ref={contentRef}>
        <div style={{ padding: "5mm", fontFamily: "'Inter', Arial, sans-serif" }}>
              ...
              {groupMeta.split("   |   ").map((item, i) => {
                ...
                return (
                  <span key={i}>
                    ...
                    <b style={{ color: "#0d5040" }}>{label}</b>{value}
                  </span>
                );
              })}
            </div>
          ...
          <div style={{
            ...
            <div>
              Total Pilgrims: <b style={{ color: "#0d5040", fontSize: "9pt" }}>{pilgrims.length}</b>
            </div>
