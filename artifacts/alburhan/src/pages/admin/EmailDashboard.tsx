...
import { Mail, RefreshCw, Send, CheckCircle2, XCircle, BarChart2, FileText, Settings, Inbox } from "lucide-react";
...
export default function EmailDashboard() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const l = await fetch(`${API}/api/notification-logs?channel=email&limit=100`, { credentials: "include" }).then(r => r.ok ? r.json() : []);
    ...
    <AdminLayout>
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-primary/10 flex items-center justify-center"><Mail size={18} className="text-primary" /></div>
              Email Dashboard
            </h1>
            <p className="text-sm text-muted-foreground mt-0.5">Email center — templates, delivery tracking, and campaign analytics</p>
          </div>
          <div className="flex gap-2">
            <Button onClick={load} variant="outline" size="sm" className="gap-1.5">
              <RefreshCw size={13} className={loading ? "animate-spin" : ""} /> Refresh
            </Button>
            <Link href="/admin/email-templates"><Button size="sm" className="gap-1.5"><FileText size={13} /> Templates</Button></Link>
          </div>
        </div>
        ...
        <div className="rounded-2xl border bg-card p-5">
          <h2 className="font-semibold text-sm mb-3">Recent Email Log</h2>
          {loading ? <p className="text-sm text-muted-foreground">Loading…</p> : logs.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-xs mt-1">Emails are logged when notifications are sent to customers</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-muted-foreground text-xs">
                    ...
                    <th className="pb-2 text-left font-medium">Subject / Event</th>
                ...
                <tbody>
                  {logs.slice(0, 10).map((l, i) => (
                    <tr key={i} className="border-b last:border-0 hover:bg-muted/30">
                      <td className="py-2 font-medium">{l.recipient || l.email || "—"}</td>
                      <td className="py-2 text-muted-foreground text-xs">{l.event_type || l.subject || "—"}</td>
                      <td className="py-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "Communication Center", href: "/admin/communication-center", icon: Mail },
