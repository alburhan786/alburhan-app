...
import { Plus, Pencil, Trash2, Search, Download, TrendingDown } from "lucide-react";
...
const EMPTY_FORM = { groupId: "", category: "misc", vendor: "", description: "", amount: "", date: new Date().toISOString().slice(0,10), paidBy: "", paymentMethod: "cash", invoiceNumber: "", notes: "" };
...
export default function ExpenseManager() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows;
  }, [expenses, catFilter, fromDate, toDate, search]);
      ...
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Track all company expenses</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={exportExcel}><Download size={14} className="mr-1.5" />Export</Button>
        ...
        <div className="bg-white rounded-xl border p-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {catFilter !== "all" && (
              <button onClick={() => setCatFilter("all")} className="text-xs text-muted-foreground underline px-2">Clear filter</button>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border p-3 flex flex-wrap gap-2 items-center">
          <div className="relative flex-1 min-w-[180px]">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search description, vendor…" className="pl-8 h-8 text-sm" />
          </div>
          ...
          {filtered.length < expenses.length && (
            <span className="text-xs text-muted-foreground">Showing {filtered.length} · {fmt(totalFiltered)}</span>
          )}
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                  ...
                  {filtered.map(e => {
                    ...
                    return (
                      <tr key={e.id} className="hover:bg-muted/20">
                        ...
                        <td className="px-4 py-2.5 text-xs capitalize">{e.paymentMethod || "—"}</td>
                        <td className="px-4 py-2.5 text-xs">{e.invoiceNumber || "—"}</td>
                        <td className="px-4 py-2.5">
                ...
                <tfoot className="bg-muted/30 border-t">
                  <tr>
                    <td colSpan={4} className="px-4 py-2.5 text-xs font-semibold text-muted-foreground">TOTAL ({filtered.length} entries)</td>
          ...
          <div className="space-y-3 mt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select value={form.paymentMethod} onChange={e => setForm(f => ({...f, paymentMethod: e.target.value}))} className="mt-1 w-full h-9 px-2 rounded border text-sm bg-background capitalize">
