  ...
  return (
    <MainLayout>
      ...
      <div className="container mx-auto px-4 -mt-16 relative z-20 pb-24">
        {/* Filters */}
        <div className="bg-white rounded-2xl shadow-xl p-4 md:p-6 mb-12 flex flex-wrap gap-4 justify-center">
          {['all', 'umrah', 'ramadan_umrah', 'hajj', 'special_hajj'].map(type => (
            <button
              key={type}
              onClick={() => setFilter(type)}
              className={`px-6 py-2.5 rounded-full text-sm font-semibold transition-all ${filter === type ? 'bg-primary text-white shadow-md' : 'bg-muted text-foreground hover:bg-primary/10 hover:text-primary'}`}
            >
              {type === 'all' ? 'All Packages' : type.replace('_', ' ').toUpperCase()}
            </button>
          ))}
        </div>
                ...
                <Card className="overflow-hidden h-full flex flex-col hover:shadow-2xl transition-shadow duration-300 border-border/50 group">
                  <div className="relative h-56 overflow-hidden">
                    <div className="absolute top-4 left-4 z-10 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-primary shadow-sm">
                      {pkg.type.replace('_', ' ').toUpperCase()}
                    </div>
